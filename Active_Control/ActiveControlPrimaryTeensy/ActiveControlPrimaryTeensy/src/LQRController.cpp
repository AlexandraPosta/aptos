...
int ravelIndex(int i, int j)
{
    return MatrixFunctions::ravelIndex(STATE_SPACE_SIZE, i, j);
}

int ravelIndex(int i, int j, int k)
{
    return MatrixFunctions::ravelIndex(NUM_SERVOS, STATE_SPACE_SIZE, i, j, k);
}
...
LQRController::LQRController()
{
    ...
    for (int gainIndex = 0; gainIndex < NUM_GAINS; gainIndex++)
    {
        for (int row = 0; row < NUM_SERVOS; row++)
        {
            for (int col = 0; col < STATE_SPACE_SIZE; col++)
            {
                availableGains[ravelIndex(gainIndex, row, col)] = avGains[gainIndex][row][col];
            }
        }
    }
    currentGain = &availableGains[0];
}

void LQRController::start()
{
    currentGain = &zeroGain[0];
};

void LQRController::performLQR(float stateSpace[STATE_SPACE_SIZE], float calculatedDeflections[NUM_SERVOS])
{
    for (int row = 0; row < NUM_SERVOS; row++)
    {
        calculatedDeflections[row] = 0.0;
        for (int col = 0; col < STATE_SPACE_SIZE; col++)
        {
            calculatedDeflections[row] += currentGain[ravelIndex(row, col)] * stateSpace[col];
        }
    }
}

void LQRController::updateGain(float stateSpace[STATE_SPACE_SIZE], GainVariables &gainVars)
{
    float velocity = gainVars.velocity;
    if (velocity < V_MC)
    {
        currentGain = &zeroGain[0];
    }
    ...
    {
        currentGainIndex = (NUM_GAINS - 1) * (velocity - V_MC) / (V_MAX - V_MC);
        currentGain = &availableGains[ravelIndex(currentGainIndex, 0, 0)];
    }
}

void LQRController::printCurrentGain()
{
}